...
### Arxiv（arXiv フィード固有の知識・投稿年月フィルタ）

- arXiv のカテゴリ別 RSS は、その日の新規投稿（`Announce Type: new`）だけでなく、
  既存論文の他カテゴリへの追加（`cross`）や改訂（`replace`）も同一フィードに含む
  （2026-09-15 に `http://export.arxiv.org/rss/cs.AI` を実地確認、634件中 new=147 /
  cross=261 / replace=226）。`replace` はフィードの日付が初版投稿日ではなく改訂日を
  指すため、何年も前に投稿された論文が「今日更新があった」という理由で毎回
  再登場し続ける。`FeedCache` の新着判定は URL の初登場時刻のみを見ており、この
  ような古い論文の再登場を弾く仕組みを持たないため、`ScriptGenerator#collect_source`
  側で `Internal::Arxiv.old?` による足切りを別途行う。
- `Announce Type` ではなく arXiv ID の YYMM（投稿年月）で足切りする方式を採用した。
  `new` の ID は原理的に常に当月のため `Announce Type` 単体では判定材料にならず、
  実害のある古い論文はほぼ `replace` から来ると実地確認できたため。IDは投稿月までしか
  わからないので、`old?` はその月の最終日を投稿日とみなして比較する（同月内の記事を
  誤って古い判定にしないための安全側の丸め）。
- `max_age_days` は `RssFeedSource` の任意属性で、設定したフィードだけ判定が働く。
  未設定なら `collect_source` は判定関数自体を呼ばないため、他フィードの収集には
  一切影響しない。
...
- 行単位で解釈する（ブロックレベルのみ。インライン強調・コードは扱わない）。
  ...
  | 種別 | マッチ | 変換 |
  |---|---|---|
  | カテゴリ見出し | `^##\s+(.+?)\s*$` | `<h3 class="news-cat">…</h3>` |
  | 記事タイトル | `^###\s+\[(.+)\]\((\S+)\)\s*$`（`[...]` は貪欲） | `<div class="news-item"><p class="news-title"><strong>…</strong></p>`。$1=タイトル/$2=URL |
  | メタ行 | `^\s*\((.+)\)\s*$` | 直近項目の `<p class="news-meta">(…)</p>` |
  | 要約行 | 上記いずれにもマッチしない空でない行 | 直近項目の `<p class="news-sum">` |
  | 空行 | — | 項目区切り（無視） |

- カテゴリ見出しは `<h3>` にする（ページの見出しアウトラインは h1 タイトル / h2「この回で
  紹介したニュース」/ h3 カテゴリ、という素直な階層になる）。記事タイトルは `<p><strong>`
  にとどめ、h4 にはしない（1 記事ごとの粒度をページの見出しアウトラインへ含めるのは
  過剰なため）。CSS が当たらない環境（RSS リーダー等）でも `<h3>`・`<strong>`・`<p>` の
  ブラウザ既定スタイルだけで見出し・強調・段落間隔が最低限つき、可読性が保たれる
  （feed.xml の `<content type="html">` は CSS を運べないため、この既定スタイルが
  唯一の頼り）。

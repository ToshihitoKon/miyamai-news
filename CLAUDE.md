...
実装が前提としている業務ルール・外部ツールの癖・ファイル間の不変条件は
[docs/background.md](docs/background.md) に集約している。コードを変更する際は、
該当項目がまだ成り立っているか確認すること。

### コメントの整理方針

ドメイン知識・不変条件・「なぜそうしたか」の背景は、コード内には一切書かず
[docs/background.md](docs/background.md) に集約する。「詳細は CLAUDE.md 参照」
「背景は docs/background.md 参照」のような参照コメントもコード側に残さない
（コードとドキュメントの往復リンクは Git のコミット履歴・grep で十分辿れる）。

- コードから自明なこと（型・命名で分かること、逐条の処理説明）はコメントにしない。
- ある挙動の「なぜ」を説明したくなったら、まず docs/background.md に該当項目が
  あるか確認し、無ければそこに追記する。既にあるなら重複させない。

...
- パージ（キャッシュからの削除）は last_fetched_at（直近でそのフィードに実際に
  ...
  再登場し二重紹介につながる。
- 初回起動（キャッシュファイルがまだ無い）時のみ、新規 entry の seen_at 初期値に
  掲載日時(date)を使う（無ければ now）。now を使うと、既存の全記事が新着扱いに
  なってしまうため。2 回目以降の新規登場は now を使う。
...
- extra フィールド（はてブのブックマーク数など）は、書き込み時はシンボルキーだが
  ...
  文字列キー前提で書くこと。
- 複数 URL を持つソース（はてブ hotentry/entrylist 等）は同じ記事が両方の URL に
  載ることがあるため、link で重複除去してから返す。
- FeedCache#fetch は複数スレッドから同時に呼んでよい（取得は並列に走り、キャッシュ
  更新のみ内部で直列化する）。
- entry の同一性キーは正規化した link（末尾スラッシュの有無を無視）。正規化前の
  URL をそのまま identity にすると同じ記事を別記事として扱ってしまう
  （feed_parser.rb#normalize_link 参照）。
...
- `feed_cache.json` と `last_fetch.json` は回をまたいで永続する状態ファイルで、
  `clean` は対象にしない（`work_globs` が列挙する回ごとの中間ファイルのみが
  削除対象）。

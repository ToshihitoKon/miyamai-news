...
進み、`--script-only`/`--generate-only` は `synthesize` 以上、`--publish-only` は
`publish` を要求する（不足する mode で実行しようとするとエラーになる）。

```sh
ruby miyamai_news.rb                  # pipeline.mode の上限まで自動的に進む
ruby miyamai_news.rb --script-only    # 台本のみ生成して停止（work/ に書き出す。synthesize以上）
ruby miyamai_news.rb --generate-only  # 生成のみ（dist/ に書き出して終了。synthesize以上）
...
`pipeline.mode: digest`（既定）のフラグなし実行は、RSS収集・AI選別・facts抽出
（ニュース要約、`work/news_facts_<date>_<slot>.txt`）までで停止する。VOICEPEAK・ffmpeg・
GCS には一切触れないため、ニュースの要約だけを手元で確認したい用途に使える。
